...
function computeRechargedEnergy(
  ...
  if (gained <= 0)
    return { energy: current, newLastRechargeAt: lastRechargeAt };
...
function buildEnergyStatus(
  ...
  return {
    ...
    maxEnergy,
    energyPercentage:
      maxEnergy > 0 ? Math.round((currentEnergy / maxEnergy) * 100) : 0,
    rechargeInfo: {
      lastRechargeAt,
      nextRechargeAt,
...
export class ConsumeEnergyUseCase {
  ...
  async execute(userId: string, amount: number = 1): Promise<EnergyStatus> {
    ...
    return this.prisma.$transaction(async (tx) => {
      const energyRow = await tx.userEnergy.findUniqueOrThrow({
        where: { userId },
      });

      // Tính toán auto-recharge trước khi consume
      const { energy: rechargedEnergy, newLastRechargeAt } =
        computeRechargedEnergy(
...
export class GetEnergyUseCase {
  ...
  async execute(userId: string): Promise<EnergyStatus> {
    const energyRow = await this.prisma.userEnergy.findUnique({
      where: { userId },
    });
    if (!energyRow) {
