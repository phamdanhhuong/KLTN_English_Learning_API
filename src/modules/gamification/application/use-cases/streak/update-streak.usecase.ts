  ...
  async execute(userId: string): Promise<UpdateStreakResult> {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const calcNextMilestone = (current: number) => {
        ...
        return {
          streakCount: next,
          daysRemaining: next - current,
          rewards: STREAK_MILESTONES[next],
        };
      };
          ...
          if (streak.currentStreak >= 3) {
            await tx.streakHistory.create({
              data: {
                ...
                startDate: new Date(
                  today.getTime() - streak.currentStreak * 86400000,
                ),
                endDate: lastDate,
                endReason: 'broken',
              },
    ...
    if (result.milestoneReached) {
      this.feedService
        .autoCreatePost(userId, 'STREAK_MILESTONE', {
