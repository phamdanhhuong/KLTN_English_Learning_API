...
export class AddXpUseCase {
  ...
  async execute(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (source === 'exercise')
        activityUpdate.exerciseCount = { increment: 1 };
    ...
    if (result.leveledUp) {
      this.feedService
        .autoCreatePost(userId, 'LEVEL_UP', { newLevel: result.newLevel })
        .catch(() => {});
    }
    // XP milestone check
    this.feedService
      .autoCreatePost(userId, 'XP_MILESTONE', { totalXp: result.newXp })
      .catch(() => {});
