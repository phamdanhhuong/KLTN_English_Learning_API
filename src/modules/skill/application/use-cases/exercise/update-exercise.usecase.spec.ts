...
describe('UpdateExerciseUseCase', () => {
  ...
  it('should update exercise prompt', async () => {
    ...
    exerciseRepository.update.mockImplementation((e: Exercise) =>
      Promise.resolve(e),
    );
  ...
  it('should throw NotFoundException when exercise not found', async () => {
    ...
    await expect(
      useCase.execute('non-existent', { prompt: 'X' }),
  ...
  it('should throw BadRequestException for invalid meta', async () => {
    ...
    await expect(useCase.execute('e1', { meta: {} as any })).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('should throw BadRequestException when position is taken', async () => {
    ...
    await expect(useCase.execute('e1', { position: 5 })).rejects.toThrow(
      BadRequestException,
    );
  });
});
